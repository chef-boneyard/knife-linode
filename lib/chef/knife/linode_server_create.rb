  ...
  class Knife
    class LinodeServerCreate < Knife
      ...
      option :linode_flavor,
             ...
             proc: proc { |f| Chef::Config[:knife][:linode_flavor] = f },
      ...
      option :linode_image,
             ...
             proc: proc { |i| Chef::Config[:knife][:linode_image] = i },
      ...
      option :linode_kernel,
             ...
             proc: proc { |i| Chef::Config[:knife][:linode_kernel] = i },
      ...
      option :linode_datacenter,
             ...
             proc: proc { |i| Chef::Config[:knife][:linode_datacenter] = i },
      ...
      @@defpass = Array.new(20, '').map { chars[rand(chars.size)] }.push('A').push('a').join

      option :ssh_password,
             short: '-P PASSWORD',
             long: '--ssh-password PASSWORD',
             proc: proc { |p| Chef::Config[:knife][:ssh_password] = p },
      ...
      def run
        ...
        # FIXME: tweakable stack_script, payment terms, and disk type
        ...
        password = locate_config_value(:ssh_password)
        msg_pair('Password', password) if password == @@defpass
      ...
      def bootstrap_for_node(server, fqdn)
        ...
        bootstrap = load_values_from_config(bootstrap)
        bootstrap = locate_values(bootstrap)
        bootstrap.config[:use_sudo] = true unless config[:ssh_user] == 'root'
        bootstrap.config[:first_boot_attributes] = locate_config_value(:json_attributes) || {}
        bootstrap
      end

      def load_values_from_config(bootstrap, values)
        VALUES_FROM_CONFIG.each do |value|
          bootstrap.config[value] == config[value]
        end

        bootsrap
      end

      def locate_values(bootstrap, values)
        VALUES_TO_LOCATE.each do |value|
          bootstrap.config[value] == locate_config_value(value)
        end
